...
export class OfflineGameEngine {
  ...
  private tickPhysics(now: number): void {
    ...
    // --- Boost drop queue processing: DISABLED — Phase A feature, not yet active ---
    // When enabled, uncomment the block below. Converts boost drops into food orbs.
  ...
  private tickSnakeMovement(snake: SnakeBase, desiredAngle: number, wantsBoost: boolean): void {
    ...
    // --- Boost drain + food drops: DISABLED — Phase A feature, not yet active ---
    // When enabled, uncomment the block below. Boost costs score and drops food from tail.
    // if (isBoosting) {
  ...
  private detectCollisions(now: number): { deadId: string; killerId?: string; cause: 'body' | 'headon' }[] {
    ...
    // --- Head-on-head collision: DISABLED — Phase A feature, not yet active ---
    // When enabled, uncomment the block below.
    // Head-on collision determines winner based on boost state and score.
    ...
    //     let aDies = false;
    //     let bDies = false;
    //     if (aTotalScore === bTotalScore) { aDies = true; bDies = true; }
    //     else if (aBoosting && !bBoosting) { bDies = true; }
    //     else if (bBoosting && !aBoosting) { aDies = true; }
    //     else { if (aTotalScore > bTotalScore) { bDies = true; } else { aDies = true; } }
    //     if (aDies && !seenDead.has(a.id)) { deaths.push({ deadId: a.id, killerId: b.id, cause: 'headon' }); seenDead.add(a.id); }
    //     if (bDies && !seenDead.has(b.id)) { deaths.push({ deadId: b.id, killerId: a.id, cause: 'headon' }); seenDead.add(b.id); }
    //   }
    // }
  ...
  private tickPostDeathPhysics(now: number): void {
    ...
    // --- Boost drop queue processing (post-death): DISABLED — Phase A feature ---
    // if (this.boostDropQueue.length > 0) {
    //   for (const drop of this.boostDropQueue) {
    //     this.foods.push({
    //       id: `food-${this.arena.id}-${this.idCounterObj.value++}`,
