  ...
  if (compact) {
    ...
    return (
      <div className="relative w-full rounded-2xl overflow-hidden border border-slate-800/60 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
        {/* Outcome strip */}
        <div className={`h-1.5 w-full ${isWin ? 'bg-gradient-to-r from-emerald-600 to-emerald-400' : 'bg-gradient-to-r from-red-700 to-red-500'}`} />
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="text-2xl leading-none" aria-hidden>{countryFlag(country)}</span>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="text-sm font-bold text-white truncate">{playerName}</span>
                {clanTag && (
                  <span className="text-[9px] font-mono text-red-400 bg-red-500/10 border border-red-500/20 px-1.5 py-0.5 rounded">[{clanTag}]</span>
                )}
              </div>
              <div className="text-[10px] font-mono text-slate-500">#{userTag} · LVL {level}</div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-800/60">
            <span className="text-[9px] font-mono text-slate-600">🐍 VENOM ARENA</span>
      ...
      <div className="relative z-10 p-5 sm:p-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-red-400 tracking-wide">VENOM ARENA</span>
          </div>
        ...
        <div className="flex items-center justify-center gap-3 mb-6">
          <span className="text-3xl leading-none" aria-hidden>{countryFlag(country)}</span>
          <div className="text-center">
            <div className="flex items-center gap-2 justify-center">
              <span className="text-xl font-bold text-white">{playerName}</span>
              {clanTag && (
                <span className="text-[10px] font-mono text-red-400 bg-red-500/10 border border-red-500/20 px-1.5 py-0.5 rounded">[{clanTag}]</span>
              )}
            </div>
            <div className="text-xs font-mono text-slate-500">#{userTag} · LVL {level}</div>
          </div>
        </div>
        ...
        <div className={`text-center mb-6 py-4 rounded-2xl border ${
          ...
          <div className={`text-3xl sm:text-4xl font-black ${isWin ? 'text-emerald-400' : 'text-red-400'}`}>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="text-center py-3 rounded-xl bg-slate-900/80 border border-slate-800/50">
            <div className="text-lg mb-0.5" aria-hidden>💀</div>
            <div className="text-lg font-bold text-amber-400">{kills}</div>
            <div className="text-[9px] font-mono text-slate-500 uppercase tracking-wider">Kills</div>
          </div>
          <div className="text-center py-3 rounded-xl bg-slate-900/80 border border-slate-800/50">
            <div className="text-lg mb-0.5" aria-hidden>🐍</div>
            <div className="text-lg font-bold text-blue-400">{snakeLength}</div>
            <div className="text-[9px] font-mono text-slate-500 uppercase tracking-wider">Length</div>
          </div>
          <div className="text-center py-3 rounded-xl bg-slate-900/80 border border-slate-800/50">
            <div className="text-lg mb-0.5" aria-hidden>⏱️</div>
            <div className="text-lg font-bold text-purple-400">{formatDuration(durationSec)}</div>
            <div className="text-[9px] font-mono text-slate-500 uppercase tracking-wider">Survived</div>
          </div>
        </div>

        {/* Footer */}
        <div className="flex items-center justify-between pt-4 border-t border-slate-800/60">
          <span className="text-[10px] font-mono text-slate-600">play · extract · dominate</span>
...
function StatPill({ icon, label, value, color }: { icon: string; label: string; value: string; color: string }) {
  return (
    <div className="text-center py-2 rounded-xl bg-slate-900/80 border border-slate-800/50">
      <div className="text-sm mb-0.5" aria-hidden>{icon}</div>
