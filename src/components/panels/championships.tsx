...
// P1: Visual styles per prize tier
const PRIZE_TIER_VISUAL: Record<
  string,
  { border: string; bg: string; glow: string; accent: string }
> = {
  RANK_1: {
...
// P0: Dynamic prize computation from rank
...
function matchCapWarning(played: number) {
  ...
  if (played >= 9900)
    return {
      ...
      label: `CRITICAL — Only ${remaining} match${remaining !== 1 ? 'es' : ''} left!`,
  ...
  if (played >= 9500)
    return {
      ...
      label: `DANGER — ${remaining} matches remaining`,
  ...
  if (played >= 9000)
    return {
      ...
      label: `CAUTION — ${remaining} matches remaining`,
  ...
  return {
    ...
    bg: '',
    label: `${remaining.toLocaleString()} Championship matches remaining this year`,
    barColor: 'from-indigo-500 to-amber-500',
  };
}
...
function ChampionshipPodium({
  ...
  entries: ChampionshipContender[];
}) {
  const top3 = entries.slice(0, 3);
  if (top3.length < 3) return null;

  // Visual order: [#2, #1 (raised), #3]
  ...
  const styles = [
    {
      ...
      place: '2ND',
      border: 'border-slate-300/30',
      bg: 'from-slate-200/5 to-slate-900',
      accent: 'text-slate-200',
      glow: 'bg-slate-300/5',
    },
    {
    ...
    },
    {
      ...
      bg: 'from-orange-950/15 to-slate-900',
      accent: 'text-orange-300',
      glow: 'bg-orange-500/5',
    },
  ];
  ...
  return (
    <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-5">
      {order.map((c, i) => (
        <div
          key={c.userTag}
          className={`relative rounded-2xl border ${styles[i].border} bg-gradient-to-b ${styles[i].bg} p-3 sm:p-4 ${i === 1 ? 'sm:-mt-2 sm:pb-6' : ''} overflow-hidden transition hover:brightness-110`}
        >
          <div
            className={`absolute top-0 right-0 w-24 h-24 ${styles[i].glow} rounded-full blur-2xl pointer-events-none`}
            aria-hidden
          />
          <div className="relative text-center">
            <div className="text-3xl sm:text-4xl mb-1">{styles[i].medal}</div>
            <div
              className={`text-[9px] font-mono font-bold ${styles[i].accent} uppercase tracking-widest`}
            >
              {styles[i].place} PLACE
            </div>
            <div className="text-xs sm:text-sm font-bold text-white mt-2 truncate">
              {countryFlag(c.country)} {c.name}
            </div>
            <div className="text-[10px] font-mono text-slate-500 mt-0.5">
              {c.userTag} · [{c.clanTag}]
            </div>
            <div className="text-sm sm:text-base font-black font-mono text-emerald-400 mt-2">
              {fmtINR(c.walletChips)}c
            </div>
            <div className="text-[10px] text-slate-500 mt-0.5">
              {c.gamesPlayed.toLocaleString()} games ·{' '}
              {fmtEfficiency(c.walletChips, c.gamesPlayed)} c/game
            </div>
...
export function Championships({ onToast }: ChampionshipsProps) {
  ...
  const [findMeResult, setFindMeResult] =
    useState<ChampionshipContender | null>(null);
  const cd = useCountdown(CHAMPIONSHIP_END_DATE);
  const listRef = useRef<HTMLOListElement>(null);

  // ── Build contenders list (demo data + injected player) ───────────────
  const contenders = useMemo<ChampionshipContender[]>(() => {
    ...
    if (registered && player) {
      ...
      if (!exists) {
        list.push({
          ...
          region: [
            'IN', 'JP', 'KR', 'SG', 'AU', 'CN', 'TW', 'TH', 'VN', 'PH', 'ID', 'MY',
          ].includes(player.country)
            ? 'APAC'
            : ['US', 'CA', 'MX'].includes(player.country)
              ? 'NA'
              : [
                  'GB', 'DE', 'FR', 'IT', 'ES', 'NL', 'PL', 'SE', 'NO', 'FI',
                  'DK', 'PT', 'AT', 'CH', 'BE', 'IE', 'CZ', 'GR',
                ].includes(player.country)
                ? 'EU'
                : 'LATAM',
          projectedPrize: '',
    ...
    return list
      .sort((a, b) => b.walletChips - a.walletChips)
      .map((c, i) => ({ ...c, rank: i + 1 }));
  }, [registered, player, gamesPlayed]);

  // ── Filtered contenders (scope + region/country + rank + search) ───────
  const filteredContenders = useMemo(() => {
    let result = contenders.filter((c) => {
      if (scope === 'REGIONAL' && region !== 'ALL' && c.region !== region)
        return false;
      if (scope === 'NATIONAL' && country !== 'ALL' && c.country !== country)
        return false;
      if (rankFilter !== 'all' && rankCategoryOf(c.rank) !== rankFilter)
        return false;
      return true;
    });
    if (search.trim()) {
      ...
      result = result.filter(
        (c) =>
          ...
          c.clanTag.toLowerCase().includes(q),
      );
    }
    return result;
  }, [contenders, scope, region, country, rankFilter, search]);

  // ── Player championship analytics ──────────────────────────────────────
  const playerEntry = useMemo(
    () =>
      registered && player
        ? contenders.find((c) => c.userTag === `#${player.userTag}`) ?? null
        : null,
    [registered, player, contenders],
  );
  const playerIndex = useMemo(
    () => (playerEntry ? contenders.indexOf(playerEntry) : -1),
    [playerEntry, contenders],
  );
  const playerAbove =
    playerIndex > 0 ? contenders[playerIndex - 1] : null;
  const playerBelow =
    playerIndex >= 0 && playerIndex < contenders.length - 1
      ? contenders[playerIndex + 1]
      : null;
  const gapAbove = playerAbove
    ? playerAbove.walletChips - (player?.bankedChips ?? 0)
    : null;
  const gapBelow = playerBelow
    ? (player?.bankedChips ?? 0) - playerBelow.walletChips
    : null;

  const top3 = useMemo(() => contenders.slice(0, 3), [contenders]);
  ...
  function handleFindMe() {
    ...
    if (!registered || !player) {
      notify('Register for the championship first!', 'error', onToast);
      return;
    }
    const myRow = listRef.current?.querySelector<HTMLElement>(
      '[data-champ-me="true"]',
    );
    if (myRow) {
      ...
      setFindMeResult(playerEntry);
      notify(
        `You're ranked #${playerEntry.rank} globally — not visible in current filter.`,
        'info',
        onToast,
      );
    } else {
  ...
  return (
    <div className="relative rounded-2xl border border-slate-800/80 bg-slate-900/60 shadow-md p-5 sm:p-6 overflow-hidden">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-950/40 via-slate-900 to-indigo-950/40 p-5 sm:p-7 border border-amber-500/30 shadow-md mb-6">
        <div
          className="absolute top-0 right-0 w-72 h-72 bg-amber-500/10 rounded-full blur-3xl pointer-events-none"
          aria-hidden
        />
        ...
        <p className="text-xs sm:text-sm text-slate-300 mt-2 max-w-3xl leading-relaxed">
          Join anytime during the year! Play up to 10,000 games. When the year
          ends, players with the maximum wallet chips across Global, Regional, and
          Country leaderboards will be awarded massive chip prizes and permanently
          inducted into the Hall of Fame on January 1st!
        </p>
        ...
        <div className="mt-5 p-4 rounded-xl bg-slate-950/70 border border-amber-500/30">
          <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 text-[10px] font-mono font-bold uppercase tracking-widest text-amber-300">
              <Timer className="w-4 h-4" /> YEAR-END FINALE &amp; JAN 1 PAYOUT
              IN:
            </span>
          ...
          <div className="grid grid-cols-4 gap-2 sm:gap-3">
            {[
              ...
              <div
                key={t.l}
                className="text-center bg-slate-900 border border-slate-800 rounded-lg py-2.5"
              >
      ...
      {!registered ? (
        <div className="rounded-2xl border border-dashed border-amber-500/40 bg-amber-950/10 p-5 mb-6 text-center">
          ...
          <p className="text-sm font-bold text-white">
            Register for the 2026 Championship
          </p>
          <p className="text-xs text-slate-400 mt-1 max-w-md mx-auto">
            Join the annual tournament to track your ranking, projected prizes,
            and compete for the Hall of Fame induction on January 1st!
          </p>
          <button
            type="button"
            onClick={handleRegister}
            className="mt-3 px-5 py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-yellow-400 hover:brightness-110 text-slate-950 font-black text-xs uppercase tracking-wider transition flex items-center justify-center gap-1.5 mx-auto"
          >
        ...
        <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-br from-amber-950/15 to-slate-950/60 p-4 sm:p-5 mb-6 shadow-md">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <span className="text-sm font-bold text-white flex items-center gap-1.5">
              <Trophy className="w-4 h-4 text-amber-400" /> My Championship
              Summary
            </span>
            <span className="text-[10px] font-mono text-amber-300 px-2 py-0.5 bg-amber-500/10 border border-amber-500/30 rounded-full">
              Global Ranking
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            ...
            <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/60">
              ...
              {(() => {
                const p = playerEntry
                  ? prizeForRank(playerEntry.rank)
                  : null;
                return p ? (
                  <>
                    <div className="text-sm font-bold text-emerald-400 mt-1">
                      +{fmtINR(p.chipsReward)}c
                    </div>
      ...
      {/* ═══ PLAYER DOSSIER (enhanced with P2 match cap warnings) ═══ */}
      <div className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4 sm:p-5 mb-6 shadow-md">
        ...
        <div className="w-full h-2 bg-slate-900 rounded-full overflow-hidden border border-slate-800 mb-3">
          <div
            className={`h-full bg-gradient-to-r ${warning.barColor} rounded-full transition-colors duration-500`}
            style={{
              width: `${Math.min(100, (gamesPlayed / MAX_GAMES) * 100)}%`,
            }}
          />
        </div>
        {/* P2: Match cap warning alerts */}
        {warning.level !== 'safe' ? (
          <div
            className={`flex items-center gap-1.5 rounded-lg px-3 py-2 mb-3 text-[11px] font-bold ${warning.bg} ${warning.color}`}
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/60">
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">
              Max chips at year-end decides rank!
            </p>
          </div>
          <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/60">
            ...
            <div className="text-sm font-bold text-white mt-1">
              {registered
                ? '✅ Registered & Active in 2026 Championship'
                : 'Free Entry | Join Anytime'}
            </div>
          </div>
          <div className="p-3 rounded-xl border border-slate-800 bg-slate-900/60 flex items-center justify-center gap-2">
            {!registered ? (
              ...
              >
                <Play className="w-3.5 h-3.5 fill-current" /> PLAY CHAMPIONSHIP
                MATCH
              </button>
            )}
          </div>
      ...
      {/* ═══ PRIZE TIERS (P1: visual differentiation + spot counts) ═══ */}
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg sm:text-xl font-black text-white flex items-center gap-2">
            <Gift className="w-5 h-5 text-amber-400" /> Jan 1st Payout &amp; Hall
            of Fame Tiers
          </h2>
          <span className="text-[10px] font-mono text-slate-500">
            Awarded automatically on 01 January
          </span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {CHAMPIONSHIP_PRIZE_TIERS.map((tier) => {
            const vis =
              PRIZE_TIER_VISUAL[tier.category] ??
              PRIZE_TIER_VISUAL.RANK_51_100;
            const spots = PRIZE_SPOTS[tier.category] ?? '';
            return (
              <div
                key={tier.category}
                className={`relative p-4 rounded-2xl border ${vis.border} ${vis.bg} shadow-md overflow-hidden`}
              >
                <div
                  className={`absolute top-0 right-0 w-32 h-32 ${vis.glow} rounded-full blur-3xl pointer-events-none`}
                  aria-hidden
                />
                <div className="relative">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <div className={`text-[10px] font-mono ${vis.accent}`}>
                      {tier.badge}
                    </div>
                    <span
                      className={`text-[9px] font-mono px-2 py-0.5 rounded-full border ${vis.border} ${vis.accent} bg-slate-950/50`}
                    >
                      {spots}
                    </span>
                  </div>
                  ...
                  {tier.hallOfFameInduction && (
                    <div className="text-[11px] text-yellow-300 mt-1 flex items-center gap-1">
                      <Award className="w-3 h-3" /> Permanent Hall of Fame
                      Inscription
                    </div>
                  )}
                </div>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search..."
              className="bg-slate-900 border border-slate-800 rounded-lg pl-7 pr-7 py-1.5 text-[10px] text-white font-mono w-28 sm:w-40 focus:outline-none focus:border-amber-500/50 placeholder:text-slate-600"
            />
            {search && (
              <button
                type="button"
          ...
          <button
            ...
            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-[10px] font-bold transition border border-amber-500/30 text-amber-300 bg-amber-500/10 hover:bg-amber-500/20"
          >
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {(
          [
            { id: 'all', label: 'All' },
            { id: 'rank1', label: '👑 #1' },
            { id: 'rank2_10', label: '🥈 2–10' },
            { id: 'rank11_50', label: '🥉 11–50' },
            { id: 'rank51_100', label: '🛡️ 51–100' },
          ] as const
        ).map((f) => (
          <button
            key={f.id}
      ...
      {/* P1: Find Me Result Card */}
      {findMeResult && (
        <div className="mb-4 p-4 rounded-xl border border-amber-500/30 bg-amber-950/15">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-amber-300 flex items-center gap-1.5">
              <Crosshair className="w-3.5 h-3.5" /> Your Global Position
            </span>
            <button
              type="button"
              onClick={() => setFindMeResult(null)}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div>
              <MicroLabel>GLOBAL RANK</MicroLabel>
              <div className="text-sm font-bold text-white font-mono">
                #{findMeResult.rank}
              </div>
            </div>
            <div>
              <MicroLabel>WALLET CHIPS</MicroLabel>
              <div className="text-sm font-bold text-emerald-400 font-mono">
                {fmtINR(findMeResult.walletChips)}c
              </div>
            </div>
            <div>
              <MicroLabel>PROJECTED PRIZE</MicroLabel>
              <div
                className={`text-xs font-bold mt-0.5 ${prizeColorForRank(findMeResult.rank)}`}
              >
                {(() => {
                  const p = prizeForRank(findMeResult.rank);
                  return p
                    ? `+${fmtINR(p.chipsReward)}c`
                    : '— Outside Top 100';
                })()}
            ...
            <div>
              <MicroLabel>GAMES PLAYED</MicroLabel>
              <div className="text-sm font-bold text-slate-300 font-mono">
                {findMeResult.gamesPlayed.toLocaleString()}
              </div>
            </div>
          </div>
        </div>
      ...
      {/* P1: Top 3 Podium (Global scope, no rank filter, no search) */}
      {scope === 'GLOBAL' && rankFilter === 'all' && !search.trim() && (
        <ChampionshipPodium entries={top3} />
      )}
      ...
      <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
        ...
        <span className="text-[9px] font-mono text-slate-500">
          {filteredContenders.length} contender
          {filteredContenders.length !== 1 ? 's' : ''}
        </span>
      </div>

      {/* P2: Responsive table wrapper */}
      <div className="rounded-2xl border border-slate-800/60 bg-slate-950/80 overflow-hidden">
        <div className="overflow-x-auto">
          <div className="min-w-[680px]">
            ...
            <ol
              ref={listRef}
              className="divide-y divide-slate-900 max-h-[60vh] overflow-y-auto va-scroll"
            >
              {filteredContenders.length === 0 ? (
                ...
                filteredContenders.map((c) => {
                  ...
                  const isDemo = !isMe;
                  const prize = prizeForRank(c.rank);
                  return (
                    <li
                      key={c.userTag + c.rank}
                      data-champ-me={isMe ? 'true' : undefined}
                      className={`grid grid-cols-12 gap-2 items-center px-4 py-3 text-sm transition-all duration-500 ${
                        isMe && findMeHighlight
                          ? 'bg-amber-500/20 border-l-2 border-amber-400 ring-1 ring-inset ring-amber-400/40'
                          : isMe
                    ...
                    >
                      ...
                      <div className="col-span-1 font-mono flex items-center gap-0.5">
                        ...
                        {isMe && (
                          <span className="text-[8px] bg-amber-500 text-black px-1 rounded font-bold ml-0.5">
                            YOU
                          </span>
                        )}
                      </div>
                      ...
                      <div className="col-span-3 min-w-0">
                        <div className="font-bold text-white truncate flex items-center gap-1.5">
                          ...
                          {isDemo && (
                            <span className="text-[7px] font-mono text-slate-500 bg-slate-800 px-1 py-px rounded shrink-0">
                              DEMO
                            </span>
                          )}
                        </div>
                      ...
                      {/* Games */}
                      <div className="col-span-1 text-right text-xs font-mono text-slate-400 tabular-nums">
                        {c.gamesPlayed.toLocaleString()}
                      </div>

                      {/* P2: Efficiency */}
                      <div className="col-span-1 text-right text-[10px] font-mono text-cyan-400/60 tabular-nums">
                        {fmtEfficiency(c.walletChips, c.gamesPlayed)}
                      </div>
                      ...
                      {/* P0: Dynamic prize (no more hardcoded projectedPrize) */}
                      <div className="col-span-2 text-right">
                        {prize ? (
                          <div className="leading-tight">
                            <div
                              className={`text-[10px] font-mono font-bold ${prizeColorForRank(c.rank)}`}
                            >
                              +{fmtINR(prize.chipsReward)}c
                            </div>
