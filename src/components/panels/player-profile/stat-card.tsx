...
export function StatCard({
  ...
  return (
    <div className="bg-slate-950/40 border border-slate-900 rounded-xl p-4 lg:p-1 flex flex-col justify-between lg:justify-start hover:border-slate-800 transition shadow">
      <div className="flex items-center justify-between text-slate-400 mb-2 lg:mb-0 lg:leading-tight">
        <span className="text-xs lg:text-[11px] font-sans">{label}</span>
        {icon}
      </div>
      <div className="lg:leading-tight">
        <span
          className={`text-xl lg:text-xs font-bold font-mono tracking-tight block ${valueClass}`}
        >
          {value}
        </span>
...
export function CapCard({
  ...
  return (
    <div className="bg-slate-900/80 p-4 lg:p-1 rounded-xl border border-slate-800 space-y-2 lg:space-y-0 lg:leading-tight">
