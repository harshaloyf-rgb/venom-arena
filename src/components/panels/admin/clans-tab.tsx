            ...
            <div className="max-h-[500px] overflow-y-auto space-y-2 pr-1 custom-scrollbar">
              {clans.map((clan) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span className="text-2xl leading-none flex-shrink-0">{clan.emblem || '🏴'}</span>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-bold text-white truncate">{clan.name}</span>
                        ...
                        <div className="flex items-center gap-3 mt-1.5">
                          ...
                          <span className="text-[10px] text-slate-600 ml-auto">{timeAgo(clan.createdAt)}</span>
                        </div>
                      </div>
                      <ChevronRight className={`h-4 w-4 flex-shrink-0 transition-transform duration-200 ${isSelected ? 'rotate-90 text-emerald-400' : 'text-slate-600 group-hover:text-slate-400'}`} />
                    </div>
                    <div className="mt-3">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] font-mono uppercase tracking-wider text-slate-600">XP</span>
                        <span className="text-[10px] font-mono text-slate-500">{formatNumber(clan.xp)} / {formatNumber(nextLevelXp)}</span>
                      </div>
            ...
            {selected ? (
              <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-6 animate-in fade-in slide-in-from-right-2 duration-300">
                ...
                <div className="flex items-start gap-4 mb-5">
                  <span className="text-4xl leading-none flex-shrink-0">{selected.emblem || '🏴'}</span>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      <h3 className="text-lg font-black text-white tracking-tight">{selected.name}</h3>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white font-bold rounded-xl text-xs transition-all cursor-pointer disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white font-bold rounded-xl text-xs transition-all cursor-pointer disabled:opacity-50"
                  >
                    <Settings className="h-3.5 w-3.5" /> Adjust Stats
                  </button>
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 hover:text-white font-bold rounded-xl text-xs transition-all cursor-pointer disabled:opacity-50"
                  >
                    {membersLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Users className="h-3.5 w-3.5" />}
                  ...
                  {!disbandConfirm ? (
                    <button
                      ...
                      disabled={actionLoading}
                      className="inline-flex items-center gap-1.5 px-3 py-2 bg-rose-950/50 hover:bg-rose-900/50 border border-rose-500/30 text-rose-400 hover:text-rose-300 font-bold rounded-xl text-xs transition-all cursor-pointer disabled:opacity-50"
                    >
                      <Trash2 className="h-3.5 w-3.5" /> Disband Clan
                    </button>
                  ) : (
                    <button
                      onClick={handleDisband}
                      disabled={actionLoading}
                      className="inline-flex items-center gap-1.5 px-3 py-2 bg-rose-600 hover:bg-rose-500 border border-rose-400 text-white font-bold rounded-xl text-xs transition-all cursor-pointer animate-pulse disabled:opacity-50"
                    >
                      <AlertTriangle className="h-3.5 w-3.5" /> Confirm Disband [${selected.tag}]
                    </button>
                  )}
                  {disbandConfirm && (
                    <button
                      onClick={() => setDisbandConfirm(false)}
                      className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-950 border border-slate-800 text-slate-400 font-bold rounded-xl text-xs transition-all cursor-pointer"
                    >
                ...
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[10px] font-mono uppercase tracking-widest text-slate-500">Experience Progress</span>
                ...
                {showMembers && (
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Members ({members.length})</span>
                    ...
                    {membersLoading ? (
                      ...
                      <p className="text-xs text-slate-500 text-center py-6">No members found.</p>
                    ) : (
                      <div className="max-h-[300px] overflow-y-auto space-y-1.5 pr-1 custom-scrollbar">
                        {members.map((m) => (
                          <div key={m.id} className="flex items-center gap-3 p-2.5 rounded-xl border border-slate-800/60 bg-slate-950/50 hover:bg-slate-900/60 transition-colors">
                            <div className="w-8 h-8 rounded-lg bg-slate-900 flex items-center justify-center border border-slate-800 text-xs overflow-hidden shrink-0">
                              {m.avatar ? (
                                ...
                                <span className="text-[10px] font-mono font-bold text-slate-400">{m.level}</span>
                              )}
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                ...
                                <span className="text-[10px] text-emerald-500">{formatNumber(m.bankedChips)}c</span>
                              </div>
                            </div>
                            <div className="flex items-center gap-1 shrink-0">
                              {m.clanRank !== 'Leader' && (
                                <button
                                  ...
                                  disabled={actionLoading}
                                  className="p-1.5 rounded-lg bg-slate-900 hover:bg-amber-500/10 border border-slate-800 hover:border-amber-500/30 text-slate-400 hover:text-amber-400 transition-all cursor-pointer disabled:opacity-50"
                              ...
                              {m.clanRank !== 'Leader' && (
                                <button
                                  ...
                                  disabled={actionLoading}
                                  className="p-1.5 rounded-lg bg-slate-900 hover:bg-rose-500/10 border border-slate-800 hover:border-rose-500/30 text-slate-400 hover:text-rose-400 transition-all cursor-pointer disabled:opacity-50"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                  <StatCard icon={<Users className="h-4 w-4" />} label="Members" value={String(selected.memberCount)} color="text-emerald-400" bg="bg-emerald-500/10" border="border-emerald-500/20" />
                  <StatCard icon={<Coins className="h-4 w-4" />} label="Banked Chips" value={formatNumber(selected.bankedChips)} color="text-amber-400" bg="bg-amber-500/10" border="border-amber-500/20" />
                  <StatCard icon={<Coins className="h-4 w-4" />} label="Total Deposited" value={formatNumber(selected.totalDeposited)} color="text-sky-400" bg="bg-sky-500/10" border="border-sky-500/20" />
                  <StatCard icon={<Trophy className="h-4 w-4" />} label="Level" value={String(selected.level)} color="text-purple-400" bg="bg-purple-500/10" border="border-purple-500/20" />
                </div>
...
function StatCard({ icon, label, value, color, bg, border }: { icon: React.ReactNode; label: string; value: string; color: string; bg: string; border: string }) {
  return (
    <div className={`rounded-xl border ${border} ${bg} p-3 flex flex-col gap-1.5`}>
...
function RankBadge({ rank }: { rank: string | null }) {
  if (rank === 'Leader') {
    return (
      <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-amber-500/15 border border-amber-500/30 text-[9px] font-bold text-amber-400">
  ...
  if (rank === 'Co-Leader') {
    return (
      <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-emerald-500/15 border border-emerald-500/30 text-[9px] font-bold text-emerald-400">
        <Shield className="h-2.5 w-2.5" />Co-Leader
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-slate-800 border border-slate-700/60 text-[9px] font-bold text-slate-400">
