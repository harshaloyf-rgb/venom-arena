            ...
            <div className="p-4 rounded-2xl border border-rose-500/20 bg-rose-500/5 space-y-3">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={warSearch} onChange={(e) => onWarSearchChange(e.target.value.toUpperCase())} placeholder="Search by name or tag..." className="w-full bg-slate-950 border border-slate-800 rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:border-rose-500/50" />
                </div>
              </div>
              {warSearch ? (
                <div className="max-h-48 overflow-y-auto va-scroll rounded-xl border border-slate-800 bg-slate-950/80">
                  {clans.filter(c => c.tag !== playerClanTag && (c.name.toLowerCase().includes(warSearch.toLowerCase()) || c.tag.includes(warSearch))).length === 0 ? (
                    ...
                    <button key={c.tag} type="button" onClick={() => onWarSearchChange(c.tag)} className={`w-full px-4 py-2.5 text-left flex items-center justify-between hover:bg-rose-500/10 transition border-b border-slate-900 last:border-0 ${warSearch === c.tag ? 'bg-rose-500/10' : ''}`}>
              ...
              {clans.find(c => c.tag === warSearch) ? (
                <div className="p-3 rounded-xl border border-rose-500/30 bg-rose-500/5 flex items-center justify-between">
                  ...
                  <button type="button" onClick={() => onWarSearchChange('')} className="p-1 rounded hover:bg-slate-800 text-slate-500 hover:text-white transition"><X className="w-3.5 h-3.5" /></button>
                </div>
              ) : null}
              <div>
                <label className="text-[10px] font-mono uppercase tracking-widest text-slate-500 block mb-1">Wager (min 1,000c per clan)</label>
                <input type="number" value={warWager} onChange={(e) => onWarWagerChange(e.target.value)} placeholder="Enter wager amount..." className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white font-mono focus:outline-none focus:border-rose-500/50" />
