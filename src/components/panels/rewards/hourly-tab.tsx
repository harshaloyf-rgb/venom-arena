  ...
  return (
    <div className="space-y-4 lg:space-y-1">
      ...
      <div className="bg-slate-950/40 rounded-2xl border border-slate-800 p-6 lg:p-2 flex flex-col items-center gap-4 lg:gap-1">
        <div className="w-20 h-20 lg:w-10 lg:h-10 rounded-full bg-cyan-500/10 border-2 border-cyan-500/30 flex items-center justify-center">
          <Clock className={`w-8 h-8 lg:w-5 lg:h-5 ${hourlyCanClaim ? 'text-cyan-400' : 'text-slate-500'}`} />
        </div>
        {hourlyCanClaim ? (
          <>
          ...
          <>
            <p className="text-sm lg:text-[11px] text-slate-400">Next claim in:</p>
            <span className="text-3xl lg:text-xl font-mono font-black text-white tabular-nums">{timeLabel(hourlyTimeLeft)}</span>
            <button type="button" disabled className="inline-flex items-center gap-2 lg:gap-1 px-6 py-3 lg:px-2 lg:py-1 bg-slate-800 border border-slate-700 text-slate-500 font-bold rounded-xl text-sm lg:text-[11px] uppercase tracking-wider cursor-not-allowed">
              <Clock className="w-5 h-5 lg:w-3 lg:h-3" /> Cooling Down
            </button>
          </>
        )}
      </div>
