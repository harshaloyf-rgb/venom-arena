...
Task ID: 1
Agent: Main Agent
Task: Read all key source files for game fix audit

Work Log:
- Read GameCanvas.tsx (504 lines) — game loop, accumulator, culling, render flow
- Read camera.ts (125 lines) — interpolated camera with alpha, zoom quantization
- Read render-snake-atlas.tsx (1531 lines) — bot fallback renderer, player atlas renderer, alpha interpolation offset
- Read collision.ts (365 lines) — swept line + proximity collision, module-level singletons
- Read engine.ts (684 lines) — gameTick, moveSnake, food eating, killSnake, createInitialState
- Read config.ts (305 lines) — all game constants
- Read spatial-hash.ts (170 lines) — flat typed array cells, object pool for queries
- Read bot-ai.ts (996 lines) — DEFAULT_BOT_MIX=13 bots, AI types, updateAllBotAI

Stage Summary:
- **Bug: foodIdRef** — engine.ts lines 566,574 reference `foodIdRef` which is NOT defined. Should be `moveCtx.nextFoodId`. This is a CRASH bug blocking both modes.
- **Bug: Bot count** — DEFAULT_BOT_MIX only has 13 bots total (2+1+2+1+4+3). Was 1000 before (per user).
- **Bug: Bot head-body jump** — renderSnakeFallback applies alpha interpolation offset (lines 816-826) to bots, but GameCanvas comment at line 262-263 says "Applying alpha to bots causes vibration". The bot render call passes alpha=1.0 but the fallback still computes the offset from prevHeadX/Y which are NOT saved for bots in GameCanvas loop (only saved for player at lines 266-267).
- **Perf: Online mode lag** — 3000 initial food, food hash rebuild every 3 ticks, food eating magnet loops over ALL food per snake per tick, no food culling to viewport
- **Perf: _moveCtx.nextFoodId not synced back** — line 587 syncs nextFoodId back, but if gameTick crashes at line 566 (foodIdRef), the id counter breaks
